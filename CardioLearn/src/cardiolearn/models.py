"""
Model definitions: baseline regressor + quantile models for uncertainty.
"""
import joblib, os
import numpy as np
...
def build_models(feature_names):
    # split numeric vs already-binary indicator columns
    num_features = [f for f in feature_names if not f.endswith(('_A','_M','_dark','_medium'))]
    # Simple preprocessor (scale numeric only)
    ...
    rf = RandomForestRegressor(random_state=0, n_estimators=400, max_depth=None, n_jobs=-1)
